...
@Service
public class SingleDirectoryMonitorService extends MonitorService {

    protected SingleDirectoryMonitorService() throws IOException {
    }

    @Override
    public void execute(Path directoryPath) {
        try {
            directoryPath.register(getWatchService(),
                    ENTRY_CREATE,
                    ENTRY_DELETE,
                    ENTRY_MODIFY);
        } catch (IOException e) {
    ...
    public void startWatcher() {
        ...
        while (getWatchKey().reset()) {
            ...
            events.forEach(this::logEventType);
